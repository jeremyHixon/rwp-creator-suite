...
(function($) {
    ...
    class SimpleAnalyticsDashboard {
        ...
        switchTab(tabId) {
            ...
            try {
                ...
                this.showError('Failed to switch tabs. Please try refreshing the page.');
            }
        }
        ...
        announceTabChange(tabId) {
            ...
            const announcement = `Switched to ${tabNames[tabId] || tabId} tab`;
        ...
        async refreshDashboard() {
            ...
            try {
                ...
                const response = await this.makeAjaxRequest('rwp_get_dashboard_metrics', {}, 10000);
                
                if (response.success) {
                    this.updateMetrics(response.data);
                    this.showSuccess(rwpAnalyticsDashboard.strings.refresh_success || 'Dashboard refreshed successfully.');
                } else {
                    throw new Error(response.data || 'Refresh failed');
                }
            } catch (error) {
                console.error('Failed to refresh dashboard:', error);
                this.showError('Failed to refresh dashboard data. Using cached data.');
            } finally {
                this.isRefreshing = false;
        ...
        updateMetrics(data) {
            ...
            this.updateElement('#content-generated', stats.content_generated_24h);
        ...
        async exportAnalytics() {
            ...
            try {
                ...
                this.showSuccess(rwpAnalyticsDashboard.strings.export_success || 'Analytics report exported successfully.');
            } catch (error) {
        ...
        async makeAjaxRequest(action, data = {}, timeout = 5000) {
            return new Promise((resolve, reject) => {
                ...
                xhr.fail((jqXHR, textStatus, errorThrown) => {
                    const error = new Error(`AJAX failed: ${textStatus} - ${errorThrown}`);
        ...
        showWelcomeMessage() {
            ...
            if (!localStorage.getItem('rwp_analytics_dashboard_visited')) {
                setTimeout(() => {
                    this.showNotification('Welcome to the Analytics Dashboard! Use Alt+1 and Alt+2 to navigate between tabs.', 'info');
                    localStorage.setItem('rwp_analytics_dashboard_visited', 'true');
                }, 1000);
            }
        }
    ...
    $(document).ready(function() {
        ...
        if ($('.rwp-analytics-dashboard').length && typeof rwpAnalyticsDashboard !== 'undefined') {
