...
class RWPConsentManager {
	...
	showBanner() {
		...
		const message = this.banner.querySelector(
			'.rwp-consent-banner-message'
		);
		const learnMoreLink = this.banner.querySelector(
		...
		if ( message ) {
			message.textContent = this.strings.message;
		}
		if ( learnMoreLink ) {
	...
	bindBannerEvents() {
		...
		document.addEventListener( 'keydown', ( e ) => {
			if (
				e.key === 'Escape' &&
				this.banner &&
				this.banner.classList.contains( 'rwp-consent-banner-visible' )
			) {
				this.setConsent( false );
			}
		} );
		...
		try {
			...
			if ( data.success ) {
				...
				document.dispatchEvent(
					new CustomEvent( 'rwpConsentChanged', {
	...
	showConfirmationMessage( message, consent ) {
		...
		confirmationDiv.className = `rwp-consent-confirmation ${
			consent ? 'rwp-consent-positive' : 'rwp-consent-neutral'
		}`;
		confirmationDiv.innerHTML = `
            <div class="rwp-consent-confirmation-content">
                <span class="rwp-consent-confirmation-icon">${
					consent ? '✓' : 'ℹ'
				}</span>
	...
	showConsentSettings() {
		...
		const acceptBtn = settingsContainer.querySelector(
			'#rwp-consent-settings-accept'
		);
		const declineBtn = settingsContainer.querySelector(
			'#rwp-consent-settings-decline'
		);
...
if ( typeof rwpConsentManager !== 'undefined' ) {
	...
	window.rwpConsent = {
		...
		setConsent: ( consent ) =>
			window.rwpConsentManagerInstance.updateConsent( consent ),
		showSettings: () =>
			window.rwpConsentManagerInstance.showConsentSettings(),
	};
}
