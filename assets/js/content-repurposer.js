...
( function () {
	...
	class ContentRepurposerApp {
		constructor( container, config = {} ) {
			...
			if ( typeof RWPStateManager === 'undefined' ) {
				console.warn(
					'ContentRepurposer: RWPStateManager dependency not loaded, using fallback'
				);
			...
			this.state = new RWPStateManager(
				'content_repurposer',
				initialState
			);
		...
		getInitialStateWithPersistence( config ) {
			...
			try {
				const stored = localStorage.getItem(
					'rwp_content_repurposer_guest_state'
				);
									...
									if ( recoveredContent ) {
										...
										const enhancedContent = {
											...
											_recoveryInfo: {
												wasRecovered: true,
												message:
													'Your previous content has been fully restored with all platforms included!',
											},
										};
										...
										this.state.setState( {
											...
											platforms:
												recoveredContent.platforms ||
												baseState.platforms,
											tone:
												recoveredContent.tone ||
												baseState.tone,
		...
		fallbackToLocalGuestData( parsedState, baseState, config ) {
			if ( parsedState.repurposedContent ) {
				...
				const convertedContent = this.convertGuestDataForLoggedInUser(
					parsedState.repurposedContent
				);
		...
		getGuestAttempts() {
			...
			try {
				const stored = localStorage.getItem(
					'content_repurposer_attempts'
				);
				const attempts = stored ? parseInt( stored, 10 ) : 0;

				const firstUseStored = localStorage.getItem(
					'content_repurposer_first_use'
				);
				const firstUse = firstUseStored
					? parseInt( firstUseStored, 10 )
					: null;
		...
		updateAttemptsDisplay() {
			const attemptsCountEl = this.container.querySelector(
				'.rwp-attempts-count'
			);
			if ( attemptsCountEl ) {
		...
		cacheElements() {
			...
			this.elements = {
				loggedInForm: container.querySelector(
					'.rwp-repurposer-logged-in'
				),
				guestForm: container.querySelector( '.rwp-repurposer-guest' ),
				guestTeaser: container.querySelector(
					'.rwp-repurposer-guest-teaser'
				),
				guestExhaustedMessage: container.querySelector(
					'.rwp-guest-exhausted-message'
				),
				contentInput: container.querySelector( '.rwp-content-input' ),
				guestContentInput: container.querySelector(
					'.rwp-guest-content-input'
				),
				characterCount:
					container.querySelectorAll( '.rwp-count-current' ),
				platformCheckboxes: container.querySelectorAll(
					'.rwp-platform-checkbox'
				),
				toneSelect: container.querySelector( '.rwp-tone-select' ),
				guestToneSelect: container.querySelector(
					'.rwp-guest-tone-select'
				),
				repurposeButton: container.querySelector(
					'.rwp-repurpose-button:not(.rwp-guest-repurpose-button)'
				),
				guestRepurposeButton: container.querySelector(
				...
				guestResultsUpgrade: container.querySelector(
					'.rwp-guest-results-upgrade'
				),
		...
		bindEvents() {
			...
			if ( this.elements.guestContentInput ) {
				this.elements.guestContentInput.addEventListener(
					'input',
					( e ) => {
						const content = e.target.value;
						this.state.setState( { content } );
				...
				this.elements.guestContentInput.addEventListener(
					...
					() => {
						setTimeout( () => {
							const content =
								this.elements.guestContentInput.value;
							this.state.setState( { content } );
							this.updateCharacterCount( content );
						}, 10 );
					}
			...
			this.elements.platformCheckboxes.forEach( ( checkbox ) => {
				checkbox.addEventListener( 'change', () => {
					const platforms = Array.from(
						this.elements.platformCheckboxes
					)
			...
			if ( this.elements.guestToneSelect ) {
				this.elements.guestToneSelect.addEventListener(
					'change',
					( e ) => {
						this.state.setState( { tone: e.target.value } );
					}
			...
			if ( this.elements.guestRepurposeButton ) {
				this.elements.guestRepurposeButton.addEventListener(
					'click',
					() => {
		...
		updateUI( providedState = null ) {
			...
			if (
				this.elements.guestContentInput &&
		...
		updateButtonState( isProcessing ) {
			...
			const buttons = [
				this.elements.repurposeButton,
				this.elements.guestRepurposeButton,
			].filter( Boolean );
		...
		async repurposeContent() {
			...
			if ( ! this.isLoggedIn ) {
				console.warn(
					'Repurpose button clicked for guest user - this should not happen'
				);
				return;
			}
			...
			try {
				...
				if ( response.success ) {
					...
					throw new Error(
						response.message || 'Failed to repurpose content'
					);
				}
			} catch ( error ) {
		...
		updateResults( repurposedContent ) {
			...
			if (
				repurposedContent._upgradeInfo &&
				repurposedContent._upgradeInfo.hasPreviewContent
			) {
				resultsHTML += `
				...
				repurposedContent._recoveryInfo &&
				repurposedContent._recoveryInfo.wasRecovered
			) {
				resultsHTML += `
			...
			Object.entries( repurposedContent ).forEach(
				( [ platform, data ] ) => {
					...
					if ( data.success && data.versions ) {
						resultsHTML += this.renderPlatformSuccess(
							platform,
							platformName,
							data.versions,
							characterLimit
						...
						resultsHTML += this.renderPlatformError(
							platform,
							platformName,
							data.error
						);
		...
		renderPlatformSuccess(
					...
					if ( isGuest && ! isTwitter && version.is_preview ) {
                        ...
                        <div class="rwp-content-version rwp-guest-preview">
                            ...
                            <div class="rwp-version-meta">
                                <span class="rwp-character-count">Preview • Full version ${
									version.estimated_length
								} / ${ characterLimit } characters</span>
                            </div>
                        </div>
                    `;
					}
					...
					return `
                        <div class="rwp-content-version">
                            ...
                            <div class="rwp-version-meta">
                                ...
                                <button type="button" class="rwp-copy-button" data-content="${ this.escapeHtml(
									version.text
								) }">
                                    Copy
                                </button>
			...
			return `
                <div class="rwp-platform-result ${
					isGuest && ! isTwitter ? 'rwp-guest-limited' : ''
				}">
                    <div class="rwp-platform-header">
                        ...
                        ${
							isGuest && ! isTwitter
								? '<span class="rwp-preview-badge">Preview</span>'
								: ''
						}
                    </div>
                ...
                <div class="rwp-platform-result rwp-result-error">
                    ...
                    <div class="rwp-platform-content">
                        <div class="rwp-error-text">
                            ${ this.escapeHtml(
								error ||
									'Failed to repurpose content for this platform'
							) }
                        </div>
                    </div>
                </div>
		...
		async loadUsageStats() {
			...
			if ( ! this.isLoggedIn || ! this.state.getState().showUsageStats ) {
				return;
			}

			try {
				const response =
					await this.makeAPIRequest( '/repurpose-usage' );
				if ( response.success ) {
		...
		updateUsageStats( stats ) {
			if ( ! this.elements.usageStats || ! stats ) {
				return;
			}

			const remaining = stats.remaining || 0;
			const quotaText =
				this.elements.usageStats.querySelector( '.quota-text' );
		...
		persistGuestState( state ) {
			...
			try {
				const stateToStore = {
					...
					lastGenerated:
						state.repurposedContent &&
						Object.keys( state.repurposedContent ).length > 0
							? Date.now()
							: null,
				};
				localStorage.setItem(
		...
		getCharacterLimits() {
			return typeof rwpContentRepurposer !== 'undefined' &&
		...
		convertGuestDataForLoggedInUser( guestRepurposedContent ) {
			if (
				! guestRepurposedContent ||
				Object.keys( guestRepurposedContent ).length === 0
			) {
				return {};
			}
			...
			Object.entries( guestRepurposedContent ).forEach(
				( [ platform, data ] ) => {
			...
			if ( hasPreviewContent ) {
				convertedContent._upgradeInfo = {
					hasPreviewContent: true,
					message:
						'Welcome! You can now regenerate this content to see the full versions.',
				};
			}

			// Debug the actual structure of converted content
			Object.entries( convertedContent ).forEach(
				( [ platform, data ] ) => {
					if ( platform.startsWith( '_' ) ) {
					}
				}
	...
	function initializeContentRepurposerApps() {
		...
		containers.forEach( ( container ) => {
			...
			if ( container.dataset.rwpInitialized ) {
				return;
			}

			const config = {
				platforms: JSON.parse(
					container.dataset.platforms || '["twitter","linkedin"]'
				),
