...
(function($) {
    ...
    function initDataSubjectRights() {
        ...
        $('#request-data-portability').on('click', handleDataPortabilityRequest);
    }
    ...
    function handleDataExportRequest() {
        ...
        wp.apiFetch({
            ...
            if (response.success) {
                ...
                showNotification(response.message || 'Failed to process export request.', 'error');
            }
        }).catch(function(error) {
            console.error('Data export error:', error);
            showNotification('An error occurred while processing your request.', 'error');
        }).finally(function() {
            hideLoadingState('#request-data-export', 'Request Data Export');
        });
    }
    ...
    function handleDataRectification() {
        ...
        if (Object.keys(corrections).length === 0) {
            showNotification('Please specify what data needs to be corrected.', 'warning');
            return;
        }
        ...
        wp.apiFetch({
            ...
            if (response.success) {
                ...
                showNotification(response.message || 'Failed to process corrections.', 'error');
            }
        }).catch(function(error) {
            console.error('Data rectification error:', error);
            showNotification('An error occurred while processing your corrections.', 'error');
        }).finally(function() {
            hideLoadingState('#submit-data-corrections', 'Submit Corrections');
        });
    }
    ...
    function handleDataErasureRequest() {
        ...
        const confirmMessage = erasureScope === 'all' 
            ? 'Are you sure you want to permanently delete ALL your data? This action cannot be undone.'
            : `Are you sure you want to delete your ${erasureScope.replace('_', ' ')} data? This action cannot be undone.`;
        ...
        wp.apiFetch({
            ...
            if (response.success) {
                ...
                showNotification(response.message || 'Failed to process deletion request.', 'error');
            }
        }).catch(function(error) {
            console.error('Data erasure error:', error);
            showNotification('An error occurred while processing your deletion request.', 'error');
        }).finally(function() {
        ...
        wp.apiFetch({
            ...
            if (response.success) {
                ...
                if (response.download_url) {
                    showDownloadLink(response.download_url, format, response.size);
                }
            } else {
                showNotification(response.message || 'Failed to generate portable export.', 'error');
            }
        }).catch(function(error) {
            console.error('Data portability error:', error);
            showNotification('An error occurred while generating your export.', 'error');
        }).finally(function() {
            hideLoadingState('#request-data-portability', 'Download My Data');
        });
    }
    ...
    function getDataCorrections() {
        ...
        $('.data-correction-field[data-type="preferences"]').each(function() {
        ...
        if ($statusDiv.length === 0) {
            const statusHtml = `
                <div id="export-status" class="data-rights-status">
                    ...
                    ${exportId ? `<p class="export-id">Export ID: ${exportId}</p>` : ''}
                </div>
            `;
            
            $('#request-data-export').closest('.tool-item').after(statusHtml);
        } else {
            $statusDiv.find('.status-indicator')
    ...
    function showDownloadLink(downloadUrl, format, size) {
        ...
        $('#request-data-portability').closest('.tool-item').after(linkHtml);
    }
