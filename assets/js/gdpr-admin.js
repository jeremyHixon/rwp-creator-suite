...
(function($) {
    ...
    function initGdprAdmin() {
        ...
        $('#run-compliance-check, #initial-compliance-check').on('click', handleRunComplianceCheck);
    ...
    function handleRunComplianceCheck(e) {
        ...
        if (!window.rwpGdprAdmin) {
            ...
            alert('Error: GDPR Admin configuration not found. Please check console for details.');
            return;
        }
        ...
        $button.text(rwpGdprAdmin.strings.runningComplianceCheck || 'Running compliance check...')
               .prop('disabled', true)
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    showNotification(rwpGdprAdmin.strings.complianceCheckComplete || 'Compliance check completed!', 'success');
                    ...
                    showNotification(response.data || rwpGdprAdmin.strings.error || 'Compliance check failed', 'error');
                }
            },
            error: function(xhr, status, error) {
                console.error('AJAX error:', {xhr: xhr, status: status, error: error});
                console.error('Response text:', xhr.responseText);
                showNotification(rwpGdprAdmin.strings.error || 'An error occurred during compliance check', 'error');
            },
            complete: function() {
    ...
    function handleExportReport() {
        ...
        // Show loading state
        $button.text(rwpGdprAdmin.strings.exportingReport)
               .prop('disabled', true);
    ...
    function handleDataCleanup() {
        ...
        // Show loading state
        $button.text(rwpGdprAdmin.strings.cleaningUpData)
               .prop('disabled', true)
               .addClass('updating-message');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    showNotification(rwpGdprAdmin.strings.dataCleanupComplete, 'success');
                } else {
                    showNotification(response.data || rwpGdprAdmin.strings.error, 'error');
                }
            },
            error: function(xhr, status, error) {
    ...
    function updateComplianceDisplay(report) {
        ...
        $('.score-status').text(report.status.replace('_', ' ').toUpperCase());
        
        // Update score class
        $('.score-number').removeClass('score-compliant score-warning score-critical score-minor_issues')
                          .addClass('score-' + report.status);

        // Update last check time
        if (report.timestamp) {
            $('.last-check-time').text(new Date(report.timestamp).toLocaleString());
        }
    }
    ...
    function initializeCharts() {
        ...
        const consentCanvas = document.getElementById('consent-breakdown-chart');
        if (consentCanvas) {
    ...
    function initConsentBreakdownChart(canvas) {
        ...
        new Chart(canvas, {
            type: 'doughnut',
            data: {
                labels: ['Basic Analytics', 'Hashtag Trends', 'Performance Benchmarking', 'Product Improvement'],
                datasets: [{
                    data: [85, 65, 78, 72],
                    backgroundColor: [
                        '#3498db',
