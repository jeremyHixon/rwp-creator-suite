...
( function ( $ ) {
	...
	function saveConsentPreferences( consentData ) {
		...
		$button
			.text( rwpGranularConsent.strings.processing || 'Processing...' )
			.prop( 'disabled', true );
		...
		wp.apiFetch( {
			...
			.then( function ( response ) {
				if ( response.success ) {
					showNotification(
						response.message || rwpGranularConsent.strings.success,
						'success'
					);
					...
					showNotification(
						response.message || rwpGranularConsent.strings.error,
						'error'
					);
				}
			} )
		...
		wp.apiFetch( {
			...
			.then( function ( response ) {
				if ( response.success && response.consents ) {
					...
					for ( const [ category, consented ] of Object.entries(
						response.consents
					) ) {
						$( `input[name="rwp_consent[${ category }]"]` ).prop(
							'checked',
							consented
						);
					}
