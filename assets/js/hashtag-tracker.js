...
class RWPHashtagTracker {
	...
	bindInputHashtagTracking( element ) {
		...
		const trackHashtags = () => {
			...
			const newHashtags = this.extractNewHashtags(
				lastValue,
				currentValue
			);
			...
			elements.forEach( ( element ) => {
				element.addEventListener( 'click', () => {
					const feature =
						...
						this.getFeatureFromElement( element );
					if ( feature ) {
	...
	bindFormTracking() {
		...
		const forms = document.querySelectorAll(
			'form[data-rwp-feature], .rwp-form'
		);
		...
		forms.forEach( ( form ) => {
			form.addEventListener( 'submit', ( e ) => {
				...
				textInputs.forEach( ( input ) => {
					const hashtags = this.extractHashtagsFromText(
						input.value
					);
	...
	extractNewHashtags( oldText, newText ) {
		...
		return newHashtags.filter(
			( hashtag ) =>
				! oldHashtags.includes( hashtag ) &&
				! this.trackedHashtags.has( hashtag )
		);
	}
	...
	getCurrentPlatform() {
		...
		const platformElements = document.querySelectorAll(
			'select[name*="platform"], input[name*="platform"]:checked'
		);
	...
	getCurrentTone() {
		...
		const toneElements = document.querySelectorAll(
			'select[name*="tone"], input[name*="tone"]:checked'
		);
	...
	getFeatureFromElement( element ) {
		...
		if (
			element.closest( '.wp-block-rwp-creator-suite-content-repurposer' )
		) {
			return 'content_repurposer';
		}
	...
	async trackHashtag( hashtag, context = {} ) {
		...
		try {
			const response = await fetch(
				`${ this.apiUrl }analytics/track-hashtag`,
				{
					method: 'POST',
					headers: {
	...
	async trackAction( action, value, context = {} ) {
		try {
			const response = await fetch(
				`${ this.apiUrl }analytics/track-action`,
				{
					method: 'POST',
					headers: {
						'Content-Type': 'application/json',
...
if ( typeof rwpHashtagTracker !== 'undefined' ) {
	...
	window.rwpTrack = {
		hashtag: ( hashtag, context ) =>
			window.rwpHashtagTrackerInstance.manualTrackHashtag(
				hashtag,
				context
			),
		action: ( action, value, context ) =>
			window.rwpHashtagTrackerInstance.manualTrackAction(
				action,
				value,
				context
			),
