...
const CharacterCounter = ({
    ...
    const getPlatformStatus = (platform) => {
        ...
        else if (percentage >= warningThreshold * 100) status = 'warning';
    ...
    const getOverallStatus = () => {
        ...
        const platformStatuses = selectedPlatforms.map(getPlatformStatus).filter(Boolean);
        
        if (platformStatuses.some(p => p.status === 'over')) {
            const overPlatforms = platformStatuses.filter(p => p.status === 'over');
            return {
                status: 'over',
                message: sprintf(
                    _n(
        ...
        if (platformStatuses.some(p => p.status === 'warning')) {
            const warningPlatforms = platformStatuses.filter(p => p.status === 'warning');
            return {
                status: 'warning',
                message: sprintf(
                    _n(
    ...
    const renderPlatformLimit = (platformStatus) => {
        ...
        const { platform, name, current, limit, remaining, percentage, status } = platformStatus;
            ...
            <div key={platform} className={`platform-limit ${status}`}>
                ...
                <div className="platform-details">
                    <span className={`remaining ${remaining < 0 ? 'negative' : ''}`}>
                        {remaining >= 0 
                            ? sprintf(__('%d characters remaining', 'rwp-creator-suite'), remaining)
                            : sprintf(__('%d characters over limit', 'rwp-creator-suite'), Math.abs(remaining))
                        }
                    </span>
                </div>
            </div>
        ...
        <div className={`rwp-character-counter ${className}`}>
                ...
                <div className={`overall-status ${overallStatus.status}`}>
                    <div className="status-header">
                        ...
                        <span className="total-count">
                            {sprintf(__('%d characters total', 'rwp-creator-suite'), characterCount)}
                        </span>
                    </div>
                </div>
            )}
            ...
            {characterCount > 0 && overallStatus.status === 'over' && (
                <div className="optimization-tips">
                    ...
                    <ul>
                        <li>{__('Try removing unnecessary words or phrases', 'rwp-creator-suite')}</li>
                        <li>{__('Use shorter synonyms where possible', 'rwp-creator-suite')}</li>
                        <li>{__('Consider breaking into multiple posts for platforms with strict limits', 'rwp-creator-suite')}</li>
                        <li>{__('Remove or reduce hashtags if they push you over the limit', 'rwp-creator-suite')}</li>
                    </ul>
                </div>
            )}
