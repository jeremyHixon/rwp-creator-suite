...
describe( 'API and Block Integration', () => {
	...
	describe( 'Instagram Analyzer Integration', () => {
		test( 'should handle authenticated user workflow', () => {
			...
			localStorage.setItem(
				'rwp_instagram_analyzer_whitelist',
				JSON.stringify( {
					data: apiData,
					timestamp: Date.now(),
				} )
		...
		test( 'should handle data flow between API and storage', () => {
			...
			localStorage.setItem(
				'rwp_instagram_analyzer_results',
				JSON.stringify( {
					data: testData,
					timestamp: Date.now(),
				} )
	...
	describe( 'Caption Writer Integration', () => {
		...
		test( 'should respect character limits', () => {
			...
			expect( twitterVersion.length ).toBeLessThanOrEqual(
				limits.twitter
			);
		} );
	} );
	...
	describe( 'Content Repurposer Integration', () => {
		test( 'should handle multi-platform repurposing', () => {
			const originalContent =
				'This is a long-form blog post about social media marketing strategies...';

			// Simulate repurposing API response
			const repurposedContent = {
				twitter: 'Key insights on social media marketing #marketing',
				linkedin:
					'Detailed professional insight about social media strategies...',
				facebook:
					'Engaging Facebook post version with call to action...',
				instagram:
					'Visual-focused Instagram caption with relevant hashtags',
			};

			localStorage.setItem(
				'rwp_content_repurposer_results',
				JSON.stringify( {
	...
	describe( 'Cross-Component Integration', () => {
		...
		test( 'should handle AJAX URL configuration consistently', () => {
			...
			expect( global.rwpInstagramAnalyzer.ajaxUrl ).toBe(
				expectedAjaxUrl
			);
			expect( global.rwpCaptionWriter.ajaxUrl ).toBe( expectedAjaxUrl );
			expect( global.rwpContentRepurposer.ajaxUrl ).toBe(
				expectedAjaxUrl
			);
		} );
		...
		test( 'should handle storage cleanup across components', () => {
			...
			localStorage.setItem(
				'rwp_instagram_analyzer_data',
				JSON.stringify( { test: 'data1' } )
			);
			localStorage.setItem(
				'rwp_caption_writer_data',
				JSON.stringify( { test: 'data2' } )
			);
			localStorage.setItem(
				'rwp_content_repurposer_data',
				JSON.stringify( { test: 'data3' } )
			);
			localStorage.setItem(
				'other_plugin_data',
				JSON.stringify( { test: 'other' } )
			);
			...
			expect(
				localStorage.getItem( 'rwp_instagram_analyzer_data' )
			).toBeNull();
			expect(
				localStorage.getItem( 'rwp_caption_writer_data' )
			).toBeNull();
			expect(
				localStorage.getItem( 'rwp_content_repurposer_data' )
			).toBeNull();
			expect(
				localStorage.getItem( 'other_plugin_data' )
			).not.toBeNull();
		} );
	} );
